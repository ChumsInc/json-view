...
import {Base16Theme, solarized, chalk, apathy, monokai, codeschool, ashes, pop, atelierDune, atelierForest, atelierHeath, atelierLakeside, atelierSeaside,
    bespin, brewer, bright, colors, embers, flat, google, eighties, grayscale, harmonic, greenscreen, isotope, hopscotch, mocha, ocean,
    paraiso, marrakesh, railscasts, tube, shapeshifter, summerfruit, tomorrow, twilight, threezerotwofour
} from "base16";
import {JSONViewContext, jvSettings} from "../src/JSONViewContext";
    ...
    return (
        <div>
            ...
            <div className="row g-3 align-items-baseline">
                ...
                <div className="col-auto">
                    <select className="form-select form-select-sm" value={themeName} onChange={changeHandler} id="jv--select-theme">
                        {Object.keys(themes).map(key => <option key={key} value={key}>{key}</option>)}
                    </select>
                </div>
                <div className="col-auto">
                    <select className="form-select form-select-sm" value={filename} onChange={(ev) => setFilename(ev.target.value) }>
                        <option value="">-</option>
                        {Object.keys(testURLs).map(key => (<option key={key} value={key}>{testURLs[key].title}</option>))}
                    </select>
                </div>
                <div className="col-auto">
                    <div className="form-check form-check-inline">
                        <input type="radio" checked={dark} id="jv--dark-toggle" className="form-check-input" onChange={() => setDark(true)}/>
                        <label className="form-check-label" htmlFor="jv--dark-toggle">Dark Mode</label>
                    </div>
                    <div className="form-check form-check-inline">
                        <input type="radio" checked={!dark} id="jv--light-toggle" className="form-check-input" onChange={() => setDark(false)}/>
